...
# with makes it so that you dont have to keep writing the $ in the pars and parms variables
# initial conditions
...
p1 = ggplot(as.data.frame(res), aes(pred, prey)) +
  geom_point() +
  labs(y="Prey",
       x="Pred") # we can see that there is not a unique relationship between predators and prey

# plot interactions x-y (pred vs prey) - use color for time)
p2 = ggplot(resl, aes(time, pop, color=animal)) +
  geom_line() +
  labs(y = "pop",
       x = "time")
