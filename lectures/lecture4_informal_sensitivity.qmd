...
# relative efficiency
# (new - base)/base 
# how much does your output change based on the baseline parameter 
# -25 = 25% decline
```

-   When testing model efficacy, it is a good idea to use hypothetical cases where if you alter an input, you know the intended effect on the model. (ie. precip = 0, streamflow must = 0. If not, something is wrong with a model)
...
site2 <- eff %>% map(~ solarpv(area = 0.1, # give it all parameters
  solar = sierraczosolar, clr = "green", 
  eunit = "kWhr", g = FALSE, etype = "direct", eff = .x)) # eff = .x tells the map function to substitute the efficiency with the values that you are piping into the map function
...
```{r sen1b, eval=T}
# structure command helps you know the structure of your output
head(str(site2))
...
# map_dfr extract the annual results 
tmp <- map_dfr(site2, `[`, c("annual")) # '[' is the function
...
#site2[[1]] will give you the list of the first parameter 
#site2[[2]]$mean will give you the mean of the second input parameter
```
...
-   [Box plots or Violin Plots of Model Output (where box shows effect of parameter uncertainty)]{style="color: green;"}
    -   you don't care what the input is, you just want to visualize the effect of the uncertainty. you're saying how variable/ uncertain is my model output.
...
# tells you for each year, what is my electricity -- but also, what is the range of my electricity 
# lets say you need min 0.006 -- most of the time you will get that. but there are some years where on the lower end you will not reach that value. 
# this informs level of risk
```
...
# the mean value across parameter uncertainty
```
...
-   **Sensitivity analysis** is a way to explore how model outputs change when inputs or parameters are varied
    -   You might know, for example, what the values are going to be. You still want to know how sensitive your outputs are to that change in values.
-   **Uncertainty analysis** is a way to quantify the uncertainty in model outputs due to uncertainty in inputs or parameters
    -   I have uncertainty in my fish population because I have uncertainty in how much hunting there will be.
        -   Vs. Sensitivity = if hunting changes x amount, fish population will vary by y amount
