...
# y is the variable to track the initial conditions
# parms in this situation is the growth rate r
Ptime = ode(y = initialrabbits, times = years, func = dexppop, parms = c(0.01))

head(Ptime)

# example of changing up time 
...
# look at the different methods for how to do the numerical integration. this is more under the hood that we dont really have to know or worry about right now (or in general lol)
help(ode)
...
# first run ODE solver --> the results do not give a dataframe
...
head(grnice)
# plot with model type as color
ggplot(grnice, aes(time, P, col = method)) +
  geom_point()
